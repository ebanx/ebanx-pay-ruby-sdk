...
describe Ebanx::Command::Direct do
  ...
  it "performs a successful request with split" do
    payment_with_split = @params
    payment_with_split[:payment][:split] = @split_rules
    response = ebanx.do_direct payment_with_split
    response.http_code.must_equal 200
    response.response['status'].must_equal 'SUCCESS'
  end
end
